        ...
        records.forEach(function(item) {
            ...
            if (!bCode && bName) {
                var found = HK_BANK_DATABASE.find(function(db) {
                    return bName.trim() === db.nameZh.trim() || bName.trim() === db.nameEn.trim() || bName.includes(db.code);
                });
                if (found) {
                    bCode = found.code;
                }
            ...
            if (bName && !seenNames.has(bName)) {
                ...
                // 如果 API 沒給 Code 且找不到對應，安全起見預設給正確對應或略過
                apiBanks.push({ code: bCode || '004', name: bName });
            }
        });

        if (apiBanks.length > 0) {
            // 優先顯示我們內置的完整核準清單，確保代碼 100% 絕對正確
            renderBankList(HK_BANK_DATABASE, true);
